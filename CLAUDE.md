...
- Annotate hot-path functions with `RTSAN_NONBLOCKING` (and explicit boundaries with `RTSAN_BLOCKING`) from `source/Utility/RealtimeAttributes.h` so RealtimeSanitizer can analyse them. The macros expand to `[[clang::nonblocking]]` / `[[clang::blocking]]` on Clang ≥ 20, nothing elsewhere.
